...
Page({
    data: {
        goodInfo:[
          ['作者:王明月','规格:180*50','类型:版画'],
          ['不支持退货','有装裱','有收藏证书'],
          ['年代:2004年','作品名称:追梦','100版']
        ],
        navName:'商品详情',
        height: 64, //header高度
        top: 0, //标题图标距离顶部距离
    ...
    goToCommon:function(e){
        wx.navigateTo({
            url: '/homeSub/pages/goodComs/goodComs?id='+e.currentTarget.dataset.id
        })
    ...
    clickTop:function(e){
        if(e.currentTarget.dataset.icon==='home'){
            wx.switchTab({
                url: '/pages/index/index/index'
            })
        }else if(e.currentTarget.dataset.icon==='message'){
            wx.navigateTo({
                url: '/homeSub/pages/message/message'
            })
        }else if(e.currentTarget.dataset.icon==='people'){
            wx.switchTab({
                url: '/pages/user/user/user'
            })

        }else if(e.currentTarget.dataset.icon==='cart'){
            wx.navigateTo({
    ...
    getStoreDetail(id) {
        ...
        sandBox.get({
            ...
            data: {
                include: 'photos,oneComment,user,attributes,comments'
            }
        }).then(res => {
            if (res.statusCode == 200) {
                res = res.data;
        ...
        sandBox.post({
            ...
            if (res.statusCode == 200) {
                ...
                if (res.status) {
                    ...
                    this.data.is_Fav?util.toast('收藏成功'):util.toast('取消收藏')
                } else {
                    wx.showModal({
                        content: res.message || '请求失败',
                        showCancel: false
